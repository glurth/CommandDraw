...
public class OrientedRectDrawCommand : DrawCommandBase
{
    public Vector2 center;
    public Vector2 halfSize;
    public float rotationRadians; // radians
    public Color color;
    ...
    public override List<PackedDrawCommand> ToDrawCommands(int objID, float tolerance = 0.001f)
    {
        // scalar radius field is used for rotation angle (shader expects radians)
        PackedDrawCommand cmd = new PackedDrawCommand(PackedDrawCommand.CMD_ORECT, center, halfSize, Vector2.zero, color, rotationRadians, 0.0f, objID);
        cmd.objectID = objID;
